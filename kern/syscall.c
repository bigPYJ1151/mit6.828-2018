...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	e->env_tf = *tf;
	e->env_tf.tf_cs |= 3;
	e->env_tf.tf_eflags |= FL_IOPL_0 | FL_IF;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if((uint32_t)va >= UTOP || (uint32_t)va % PGSIZE != 0 || (perm | PTE_SYSCALL) != PTE_SYSCALL)
		return -E_INVAL;
	...
	if(page_insert(e->env_pgdir, pg, va, perm | PTE_U) < 0){
		page_free(pg);
		return -E_NO_MEM;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((uint32_t)srcva >= UTOP || (uint32_t)dstva >= UTOP || (uint32_t)srcva % PGSIZE != 0 || (uint32_t)dstva % PGSIZE != 0 || (perm | PTE_SYSCALL) != PTE_SYSCALL)
		return -E_INVAL;
	...
	if((pg = page_lookup(src->env_pgdir, srcva, &src_entry)) == NULL)
		return -E_INVAL;

	if((perm & PTE_W) && !(*src_entry & PTE_W))
		return -E_INVAL;
	
	if(page_insert(dst->env_pgdir, pg, dstva, perm | PTE_U) < 0)
		return -E_NO_MEM;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uint32_t)srcva < UTOP && (uint32_t)target->env_ipc_dstva < UTOP){
		if ((PGOFF(srcva) || (perm | PTE_SYSCALL) != PTE_SYSCALL))
			return -E_INVAL;

		struct PageInfo* pg;
		pte_t* pte_entry;
	
		if (!(pg = page_lookup(curenv->env_pgdir, srcva, &pte_entry)) || ((perm & PTE_W) && ((*pte_entry & PTE_W) == 0)))
			return -E_INVAL;

		if ((r = page_insert(target->env_pgdir, pg, target->env_ipc_dstva, PTE_U | perm)) < 0)
			return r;
...
// Set priority of current environment.
static int
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
		...
		case SYS_netpacket_try_send:
			ret = sys_netpacket_try_send((void*)a1, a2);
			return ret;
		case SYS_netpacket_try_receive:
			ret = sys_netpacket_try_receive((void*)a1, a2);
			return ret;
