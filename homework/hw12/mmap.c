...
static double *sqrts;
static double *prev = NULL;
static double *cur = NULL;
...
handle_sigsegv(int sig, siginfo_t *si, void *ctx)
{
  ...
  if (prev && (munmap((void*)prev, page_size) == -1)){
    fprintf(stderr, "Coundn't munmap() region for sqrt table; %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }

  cur = (double*)align_down(fault_addr, page_size);
  cur = mmap(cur, page_size, PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (cur < 0){
    fprintf(stderr, "Coundn't mmap() region for sqrt table; %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }

  calculate_sqrts(cur, (cur - sqrts), page_size / 8);
  prev = cur;
  return;
}
