...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	struct OpenFile *f;
	size_t n = req->req_n > PGSIZE ? PGSIZE : req->req_n;

	if ((r = openfile_lookup(envid, req->req_fileid, &f)) < 0)
		return r;
	
	if ((r = file_read(f->o_file, (void*)ret->ret_buf, n, f->o_fd->fd_offset)) < 0)
		return r;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if ((r = file_write(f->o_file, (void*)req->req_buf, req->req_n, f->o_fd->fd_offset)) < 0)
		return r;
	f->o_fd->fd_offset += r;
